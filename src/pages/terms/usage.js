...
export default function TermsPage( props ) {
    const { data: { service: data } } = useTerms( props?.data );
    const router = useRouter();
    const isPopup = decodeURI( router.asPath ) === '/이용약관/seamless';
    ...
    const [content, setContent] = React.useState(data?.[current]?.content || '');
    ...
    return (
        <>
        <Header useHome={!isPopup} title="이용약관" />
